...
class Scanner {
    ...
    private func unicodeRange() -> Token {
        ...
        var hexValue = ""
        while peek1.isHexDigit && hexValue.characters.count < 6 {
            hexValue.append(advance())
        }
        
        while peek1 == "?" && hexValue.characters.count < 6 {
            hexValue.append(advance())
        }
        
        if hexValue.hasSuffix("?") {
            hexStart = String(hexValue.characters.map { $0 == "?" ? "0" : $0 })
            hexEnd = String(hexValue.characters.map { $0 == "?" ? "F" : $0 })
        } else {
            hexStart = hexValue
            if peek1 == "-" && peek2.isHexDigit {
                hexEnd = ""
                _ = advance()
                while peek1.isHexDigit && hexEnd.characters.count < 6 {
        ...
        return .unicodeRange(start: Int(hexStart, radix: 16)!, end: Int(hexEnd, radix: 16)!)
    }
